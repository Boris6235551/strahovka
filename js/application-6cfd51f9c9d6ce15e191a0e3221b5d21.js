...
$(function () {
    ...
    $('.tel a:last-child, .request_tel a:last-child').click(function(){
    ...
    $('.help').click(function(){
        $(this).find('.mini_popup').fadeToggle();
    });
    $('.popup_map .close_text').click(function(){
        $(this).parent().toggleClass('active');
        $('.hide_popup').fadeToggle();
    });
    $('.close_text').on('click', function (event) {
        if ($('.popup_map').hasClass('active')) {
    ...
    /* Selects */
    $(".country_select").ikSelect({
        customClass: "country_select",
        ddCustomClass: "country_select",
        autoWidth: false
    });

    $(".age_select").ikSelect({
        customClass: "age_select",
        ddCustomClass: "age_select",
        autoWidth: false
    });
})
;
...
(function (Controllers) {
    ...
    var Model = (function () {
        function Model() {
            ...
            this.name.subscribe(function () {
                this.display_name_should_be_present(false);
                this.submit_disabled(false);
            }, this);
            this.phone = ko.observable("");
            this.phone.subscribe(function () {
                this.display_phone_should_be_present(false);
                this.submit_disabled(false);
            }, this);
            this.auto_model = ko.observable();
            this.variant_vozmeshenija = ko.observable();
            this.issue_year = ko.observable();
            this.price = ko.observable("");
            this.price.subscribe(function () {
                this.display_price_should_be_present(false);
                this.display_price_should_be_number(false);
            }, this);
            this.affected_area = ko.observable();
            this.drivers = ko.observableArray([
                new Driver()
            ]);
            this.drivers_amount = ko.observable("Один");
            this.single_driver = ko.computed(function () {
                return this.drivers_amount() == "Один";
            }, this);
            this.single_driver.subscribe(function (value) {
                if(value) {
                    var old_values = _this.drivers.splice(0, 1);
                    _this.drivers.removeAll();
                    if(old_values.length > 0) {
                        _this.drivers(old_values);
                    }
                }
            });
            this.name_should_be_present = ko.computed(function () {
                return this.name().length == 0;
            }, this);
            this.display_name_should_be_present = ko.observable(false);
            this.phone_should_be_present = ko.computed(function () {
                return this.phone().length == 0;
            }, this);
            this.display_phone_should_be_present = ko.observable(false);
            this.price_should_be_present = ko.computed(function () {
                return this.price().length == 0;
            }, this);
            this.display_price_should_be_present = ko.observable(false);
            this.price_should_be_number = ko.computed(function () {
                return this.price().length > 0 && !this.price().match(/^[0-9]+$/);
            }, this);
            this.display_price_should_be_number = ko.observable(false);
            this.total_min = ko.observable();
            this.total_max = ko.observable();
            this.submit_disabled = ko.observable(false);
        }
        Model.prototype.addDriver = function () {
            if(!this.single_driver()) {
                this.drivers.push(new Driver());
            }
        };
        Model.prototype.displayCalcErrors = function () {
            this.display_price_should_be_present(this.price_should_be_present());
            this.display_price_should_be_number(this.price_should_be_number());
        };
        Model.prototype.displaySubmitErrors = function () {
            this.display_name_should_be_present(this.name_should_be_present());
            this.display_phone_should_be_present(this.phone_should_be_present());
        };
        Model.prototype.submitRequest = function () {
            this.displaySubmitErrors();
            if(this.name_should_be_present() || this.phone_should_be_present()) {
                this.submit_disabled(true);
                return;
            }
            var komodel = ko.toJS(this);
            var data = {
            };
            data["request[name]"] = komodel.name;
            data["request[phone]"] = komodel.phone;
            data["request[auto_model]"] = komodel.auto_model;
            data["request[variant_vozmeshenija]"] = komodel.variant_vozmeshenija;
            data["request[issue_year]"] = komodel.issue_year;
            data["request[price]"] = komodel.price;
            data["request[affected_area]"] = komodel.affected_area;
            data["request[drivers_amount]"] = komodel.drivers_amount;
            $.each(komodel.drivers, function (index, driver) {
                data["request[drivers][" + index + "][age]"] = driver.age;
                data["request[drivers][" + index + "][experience]"] = driver.experience;
            });
            var url = $("form#request-form").attr("action"), complete_url = $("form#request-form").data("complete-url");
            $.ajax({
                url: url,
                type: "POST",
                data: data,
    ...
    var KaskoRequest = (function () {
        function KaskoRequest() {
            this.model = new Model();
            ko.applyBindings(this.model);
        }
        return KaskoRequest;
    })();    
    function SetupKaskoRequest() {
        new KaskoRequest();
    }
    Controllers.SetupKaskoRequest = SetupKaskoRequest;
})(Controllers || (Controllers = {}));
var Main;
(function (Main) {
    function main() {
        if($("body.kasko_request").length) {
            Controllers.SetupKaskoRequest();
        }
    }
...
// выделяет активный пункт верхнего меню

$(function() {
  $('.top_nav [href]').each(function() {
    if (this.href == window.location.href) {
      $(this).addClass('active');
    }
  });

// приводит к правильной форме ввод телефонного номера

$(function(){
	$(".telefon input[type=tel]").mask("+375-(99)-999-99-99");
});
});